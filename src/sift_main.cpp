...
init_ok_(false)
{
  ...
  // image_sub_=it_.subscribe("/hummingbird/svo/image",2,&SiftMain::ImageCb,this);
  ...
  ss_= nh_.advertiseService("receive_image", &SiftMain::callback,this);  
  // 订阅和发布主题,用公有的命名空间
  for(i=0;i<UAV_NUM;i++){
    // 单一函数
    // image_subs_[i]=it_.subscribe(sub_strs_[i],1,&SiftMain::ImageCb2,this);
    // 多个函数
    // 详细见 http://blog.csdn.net/sunfc_nbu/article/details/52881656
    image_subs_[i]=it_.subscribe(sub_strs_[i],1,boost::bind(&SiftMain::ImageCb,this,_1,i));
  ...
  // std::cout <<"wtf???????????????????????" << std::endl;
  // 定时器,0.1s
  timer_ = nh_.createTimer(ros::Duration(0.5), &SiftMain::timerCallback,this);
}

bool SiftMain::callback(std_srvs::Trigger::Request& request, std_srvs::Trigger::Response& response)
{  
  this->init_ok_=true;
...
void SiftMain::timerCallback(const ros::TimerEvent&){
  if(!init_ok_)return ;
  int i;
  for(i=0;i<UAV_NUM;i++){
    if(i==uav_index_)continue;
...
void SiftMain::ImageCb(const sensor_msgs::ImageConstPtr& msg,const int& index)  
{  
  if(!init_ok_)return;
  ...
  {  
    ...
    siftDetector->detectAndCompute(mats_[index],Mat(),
...
bool SiftMain::pose_estimation_2d2d ( std::vector<cv::KeyPoint> keypoints_1,
